...
<div class="content">
    <div class="container-fluid">
                        ...
                        <tbody>
                            <tr>
                                <td class="text-center">
                                    <button type="button" class="btn btn-outline-success btn-lg fw-bold carpet-stat-btn" data-filter="Paid">
                                        {{ number_format($totalPaid, 2) }}
                                    </button>
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-outline-danger btn-lg fw-bold carpet-stat-btn" data-filter="Not Paid">
                                        {{ number_format($totalUnpaid, 2) }}
                                    </button>
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-outline-primary btn-lg fw-bold carpet-stat-btn" data-filter="">
        ...
        <div class="d-flex justify-content-end mb-3">
            <a href="{{ route('reports.carpets.downloadMonth', ['month' => $month, 'year' => $year]) }}"
...
$(document).ready(function() {
    ...
    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
        if (settings.nTable.id !== 'allCarpetsTable') return true;
        if (!window.carpetPaymentFilter) return true;

        var cellNode = allTable.cell(dataIndex, 4).node();
        var cellFilter = $(cellNode).attr('data-filter');
        return cellFilter === window.carpetPaymentFilter;
    });

    window.carpetPaymentFilter = '';
    ...
    $('.carpet-stat-btn').on('click', function() {
        ...
        $('.carpet-stat-btn').each(function() {
            ...
            if (f === 'Paid') {
                $(this).addClass(window.carpetPaymentFilter === 'Paid' ? 'btn-success' : 'btn-outline-success');
            } else if (f === 'Not Paid') {
                $(this).addClass(window.carpetPaymentFilter === 'Not Paid' ? 'btn-danger' : 'btn-outline-danger');
            } else {
                $(this).addClass(window.carpetPaymentFilter === '' ? 'btn-primary' : 'btn-outline-primary');
            }
        });
